...
#For calculating using theta values
import math
...
class Annotator:
    ...
    def poly_label(self, poly, theta, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        # if self.pil or not is_ascii(label):
        ...
        # else:
            ...
            #For plotting polygon, not needed here
            #poly_list = np.array([[poly[0], poly[1]], [poly[2], poly[3]], [poly[4], poly[5]], [poly[6], poly[7]]], np.int32)
            ...
            if label:
                ...
                start_point = (center_xVal, center_yVal)
                
                non_rot_width = xmax-xmin
                non_rot_height = ymax-ymin

                im_height, im_width, im_channels = self.im_cv2.shape

                sign_of_theta = np.sign(theta)
                ...
                if int(sign_of_theta[0]) == -1:
                    ...
                    end_yVal = int(center_yVal + (length * math.sin(theta)))
                else:
